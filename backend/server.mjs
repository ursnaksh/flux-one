...
const db = new DatabaseSync(join(dataDir, 'flux-one.db'));
...
let secret = db.prepare('SELECT value FROM settings WHERE key = ?').get('jwt_secret')?.value;
if (!secret) {
...
function authenticatedUser(req) {
  ...
  try {
    ...
    return claims.exp >= Math.floor(Date.now() / 1000) ? db.prepare('SELECT * FROM users WHERE id = ?').get(claims.sub) : null;
  } catch { return null; }
}
...
const server = createServer(async (req, res) => {
  ...
  try {
    ...
    if (req.method === 'GET' && route === '/health') return send(res, 200, { status: 'ok', database: 'SQLite' });
    ...
    if (req.method === 'POST' && route === '/api/v1/auth/register') {
      ...
      db.prepare('INSERT INTO users (id, email, display_name, password_hash, password_salt, created_at) VALUES (?, ?, ?, ?, ?, ?)').run(user.id, user.email, user.display_name, user.password_hash, user.password_salt, user.created_at);
    ...
    if (req.method === 'GET' && route === '/api/v1/auth/me') { const user = requireUser(req, res); if (user) send(res, 200, userView(user)); return; }
    if (route === '/api/v1/profile') {
      ...
      if (req.method === 'PUT') {
        ...
        db.prepare('UPDATE users SET display_name = ?, prn = ?, roll_number = ?, batch = ? WHERE id = ?').run(displayName, prn, roll, batch, user.id);
        return send(res, 200, userView(db.prepare('SELECT * FROM users WHERE id = ?').get(user.id)));
      }
    }
    if (req.method === 'GET' && route === '/api/v1/dashboard/overview') {
      const user = requireUser(req, res); if (!user) return;
      const stats = db.prepare("SELECT COALESCE(SUM(duration_seconds), 0) seconds, COUNT(*) completed FROM study_sessions WHERE user_id = ? AND state = 'completed'").get(user.id);
    ...
    if (req.method === 'POST' && route === '/api/v1/sessions/start') {
      const user = requireUser(req, res); if (!user) return; const data = await body(req); const course = String(data.course_code || '').trim(); const topic = String(data.topic || '').trim();
      ...
      db.prepare('INSERT INTO study_sessions (id, user_id, course_code, topic, state, started_at, last_heartbeat_at, paused_seconds) VALUES (?, ?, ?, ?, ?, ?, ?, ?)').run(session.id, session.user_id, session.course_code, session.topic, session.state, session.started_at, session.last_heartbeat_at, 0);
    ...
    if (req.method === 'POST' && sessionAction) {
      const user = requireUser(req, res); if (!user) return; const [, id, action] = sessionAction; const session = db.prepare('SELECT * FROM study_sessions WHERE id = ? AND user_id = ?').get(id, user.id);
      if (!session) return fail(res, 404, 'Study session not found.'); const timestamp = now();
      if (action === 'heartbeat') db.prepare('UPDATE study_sessions SET last_heartbeat_at = ? WHERE id = ?').run(timestamp, id);
      if (action === 'pause' && session.state === 'active') db.prepare("UPDATE study_sessions SET state = 'paused', paused_at = ? WHERE id = ?").run(timestamp, id);
      if (action === 'resume' && session.state === 'paused') { const seconds = Math.max(0, Math.round((Date.now() - Date.parse(session.paused_at)) / 1000)); db.prepare("UPDATE study_sessions SET state = 'active', paused_at = NULL, paused_seconds = paused_seconds + ? WHERE id = ?").run(seconds, id); }
      if (action === 'complete') { const data = await body(req); const rating = Number(data.focus_rating); if (!Number.isInteger(rating) || rating < 1 || rating > 5) return fail(res, 422, 'Focus rating must be 1 to 5.'); const currentPause = session.state === 'paused' ? Math.max(0, Math.round((Date.now() - Date.parse(session.paused_at)) / 1000)) : 0; const paused = Number(session.paused_seconds) + currentPause; const duration = Math.max(0, Math.round((Date.now() - Date.parse(session.started_at)) / 1000) - paused); db.prepare("UPDATE study_sessions SET state = 'completed', paused_at = NULL, paused_seconds = ?, completed_at = ?, duration_seconds = ?, focus_rating = ?, reflection = ? WHERE id = ?").run(paused, timestamp, duration, rating, String(data.reflection || '').trim() || null, id); }
      return send(res, 200, sessionView(db.prepare('SELECT * FROM study_sessions WHERE id = ?').get(id)));
    }
    if (req.method === 'GET' && route === '/api/v1/sessions/history') { const user = requireUser(req, res); if (user) send(res, 200, db.prepare('SELECT * FROM study_sessions WHERE user_id = ? ORDER BY started_at DESC').all(user.id).map(sessionView)); return; }
    if (route === '/api/v1/assignments') {
      const user = requireUser(req, res); if (!user) return;
      if (req.method === 'GET') return send(res, 200, db.prepare('SELECT * FROM assignments WHERE user_id = ? ORDER BY created_at DESC').all(user.id));
      if (req.method === 'POST') { const data = await body(req); const course = String(data.course_code || '').trim(); const title = String(data.title || '').trim(); if (!course || !title) return fail(res, 422, 'Course code and title are required.'); const assignment = { id: randomUUID(), user_id: user.id, course_code: course, topic: data.topic || null, title, due_at: data.due_at || null, priority: data.priority || 'med', status: 'pending', created_at: now(), updated_at: now() }; db.prepare('INSERT INTO assignments (id,user_id,course_code,topic,title,due_at,priority,status,created_at,updated_at) VALUES (?,?,?,?,?,?,?,?,?,?)').run(assignment.id, assignment.user_id, assignment.course_code, assignment.topic, assignment.title, assignment.due_at, assignment.priority, assignment.status, assignment.created_at, assignment.updated_at); return send(res, 201, assignment); }
    }
    const assignmentAction = route.match(/^\/api\/v1\/assignments\/([^/]+)\/toggle$/);
    if (req.method === 'PATCH' && assignmentAction) { const user = requireUser(req, res); if (!user) return; const assignment = db.prepare('SELECT * FROM assignments WHERE id = ? AND user_id = ?').get(assignmentAction[1], user.id); if (!assignment) return fail(res, 404, 'Assignment not found.'); const status = assignment.status === 'completed' ? 'pending' : 'completed'; db.prepare('UPDATE assignments SET status = ?, updated_at = ? WHERE id = ?').run(status, now(), assignment.id); return send(res, 200, db.prepare('SELECT * FROM assignments WHERE id = ?').get(assignment.id)); }
    if (route === '/api/v1/notes') {
      const user = requireUser(req, res); if (!user) return;
      if (req.method === 'GET') { const code = url.searchParams.get('course_code'); const rows = code ? db.prepare('SELECT * FROM notes WHERE user_id = ? AND course_code = ? ORDER BY created_at DESC').all(user.id, code) : db.prepare('SELECT * FROM notes WHERE user_id = ? ORDER BY created_at DESC').all(user.id); return send(res, 200, rows); }
      if (req.method === 'POST') { const data = await body(req); const course = String(data.course_code || '').trim(); const topic = String(data.topic || '').trim(); const title = String(data.title || '').trim(); const content = String(data.content || '').trim(); if (!course || !topic || !title || !content) return fail(res, 422, 'Course code, topic, title, and content are required.'); const note = { id: randomUUID(), user_id: user.id, course_code: course, topic, title, content, created_at: now(), updated_at: now() }; db.prepare('INSERT INTO notes (id,user_id,course_code,topic,title,content,created_at,updated_at) VALUES (?,?,?,?,?,?,?,?)').run(note.id, note.user_id, note.course_code, note.topic, note.title, note.content, note.created_at, note.updated_at); return send(res, 201, note); }
    }
    const noteMatch = route.match(/^\/api\/v1\/notes\/([^/]+)$/);
    if (req.method === 'DELETE' && noteMatch) { const user = requireUser(req, res); if (!user) return; const result = db.prepare('DELETE FROM notes WHERE id = ? AND user_id = ?').run(noteMatch[1], user.id); return result.changes ? res.writeHead(204).end() : fail(res, 404, 'Note not found.'); }
    if (route === '/api/v1/enrollments') { const user = requireUser(req, res); if (!user) return; if (req.method === 'GET') return send(res, 200, db.prepare('SELECT * FROM enrollments WHERE user_id = ? ORDER BY course_code').all(user.id)); if (req.method === 'POST') { const data = await body(req); const code = String(data.course_code || '').trim(); const name = String(data.course_name || '').trim(); if (!code || !name) return fail(res, 422, 'Course code and course name are required.'); const record = { id: randomUUID(), user_id: user.id, course_code: code, course_name: name, term: data.term || null, created_at: now() }; db.prepare('INSERT INTO enrollments (id,user_id,course_code,course_name,term,created_at) VALUES (?,?,?,?,?,?) ON CONFLICT(user_id,course_code) DO UPDATE SET course_name = excluded.course_name, term = excluded.term').run(record.id, record.user_id, record.course_code, record.course_name, record.term, record.created_at); return send(res, 201, record); } }
